...
void top_level_task(const Task *task, const std::vector<PhysicalRegion> &regions,
  ...
  Rect<3> launch_bounds(Point<3>(0, 0, 0),
                        Point<3>(num_points - 1, num_points - 1, num_points - 1));
  ...
  for (int i = 0; i < num_points; i++) {
    for (int j = 0; j < num_points; j++) {
      for (int k = 0; k < num_points; k++) {
        auto result = fm.get_result<long long>(Point<3>(i, j, k));
        total += result;
      }
    }
  }
...
long long index_space_task(const Task *task, const std::vector<PhysicalRegion> &regions,
  ...
  assert(task->index_point.get_dim() == 3);
  ...
  long long z = task->index_point.point_data[2];
  return x * y * z;
}
