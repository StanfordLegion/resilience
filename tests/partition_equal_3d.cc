...
int sum(const Task *task, const std::vector<PhysicalRegion> &regions, Context ctx,
  ...
  const FieldAccessor<READ_ONLY, int, 3> acc(pr, 0);
  auto domain =
      runtime->get_index_space_domain(ctx, task->regions[0].region.get_index_space());

  int total = 0;
  for (PointInRectIterator<3> pir(domain); pir(); pir++) {
    total += acc[*pir];
  }
  return total;
}
...
void write_region(const Task *task, const std::vector<PhysicalRegion> &regions,
  ...
  PhysicalRegion pr = regions[0];
  const FieldAccessor<READ_WRITE, int, 3> acc(pr, 0);
  auto domain =
      runtime->get_index_space_domain(ctx, task->regions[0].region.get_index_space());
  for (PointInRectIterator<3> pir(domain); pir(); pir++) {
...
void top_level(const Task *task, const std::vector<PhysicalRegion> &regions, Context ctx,
  ...
  int n = 2;
  Rect<3> color_bounds(Point<3>(0, 0, 0), Point<3>(n - 1, n - 1, n - 1));
  ...
  LogicalRegion lsr = runtime->get_logical_subregion_by_color(ctx, lp, Point<3>(1, 1, 1));
