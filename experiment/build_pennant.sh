...
SAVEOBJ=1 STANDALONE=1 OBJNAME=./pennant.checkpoint $root_dir/../legion/language/regent.py $root_dir/../legion/language/examples/pennant.rg -fpredicate 0 -fflow 0 -fopenmp 0 $REGENT_GPU_FLAGS -fcheckpoint 1

cp $root_dir/../build/src/liblegion_resilience.so .

cp $root_dir/$MACHINE/*_pennant*.sh .
